...
import { Search, Plus, QrCode, Edit, Trash2, ToggleLeft, ToggleRight } from "lucide-react";
...
const RoomManagement = () => {
  ...
  const [viewingQrCodeUrl, setViewingQrCodeUrl] = useState("");
  const [viewingQrCodeRoomName, setViewingQrCodeRoomName] = useState("");
  ...
  const handleRoomOperationSuccess = () => {
  ...
  const handleDeleteRoom = async (room: Room) => {
    try {
      ...
      toast({ title: "Room Deleted", description: `Room '${room.name}' has been successfully deleted.` });
  ...
  const handleViewQrCode = (qrCodeUrl: string, roomName: string) => {
    setViewingQrCodeUrl(qrCodeUrl);
    setViewingQrCodeRoomName(roomName);
    ...
    <div className="space-y-6">
                ...
                rooms.map((room) => (
                  <TableRow key={room.id}>
                    ...
                    <TableCell className="text-right space-x-2">
                      {room.qr_code && (
                        <Button variant="outline" size="sm" onClick={() => handleViewQrCode(room.qr_code!, room.name)}>
      ...
      {isAddRoomDialogOpen && (
        <RoomFormDialog
          ...
          onSaveSuccess={handleRoomOperationSuccess}
        />
      )}
      ...
      {isQrCodeDialogOpen && (
        <RoomQrCodeDialog
          ...
          qrCodeUrl={viewingQrCodeUrl}
          roomName={viewingQrCodeRoomName}
        />
      )}
    </div>
  );
